...
import { bufferizeCandidates, getSimulcastInfo, noop, removeFIDFromOffer, sleep, logger } from './util';
import type { WebRtcPeerMode, WebRtcPeerOptions } from './type';
...
export class WebRtcPeer extends EventEmitter {
  ...
  public getLocalStream(index = 0) {
    return this.getLocalStreams()[index]
  }
  ...
  static cancelChooseDesktopMedia (id: string) {
    logger.log(`Cancel Choose Desktop Media id -> ${id}`)
  }
}
